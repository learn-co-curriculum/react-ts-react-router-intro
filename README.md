...
- Understand how client-side routing works and how it differs from server-side
  routing
- Use the Location and History API in JavaScript/TypeScript
...
function NavBar() {
  function navigate(e: React.MouseEvent<HTMLAnchorElement>) {
    ...
    e.preventDefault();
    // use type assertion to type the event target as an HTMLAnchorElement
...
React Router provides similar functionality with its own custom navigation API
that lets it listen for changes to the URL and re-render your application with
the correct component based on the new URL.
